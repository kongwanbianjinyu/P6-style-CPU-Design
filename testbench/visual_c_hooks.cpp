...
void setup_gui(FILE *fp){
  ...
  if(has_colors()){
    ...
    init_pair(1,COLOR_BLUE,COLOR_WHITE);    // shell background
    ...
    init_pair(8,COLOR_BLUE, COLOR_WHITE); // "VTUBE" logo
  }
  curs_set(0);
  noecho();
  cbreak();
  ...
  arf_win = create_newwin(34,25,18,COLS-25,5);
  ...
  // instantiate a window for the Map Table on the right side
  mt_win = create_newwin(34,30,LINES-7-34,90,5);
  mvwprintw(mt_win,0,8,"Map Table");
  i=0;
  char mt_tmp_buf[32];
  for (; i < NUM_ARF; i++) {
    sprintf(mt_tmp_buf, "x%02d: ROB#", i);
    mvwprintw(mt_win,i+1,1,mt_tmp_buf);
  }
  wrefresh(mt_win);

  //instantiate window to visualize instructions in pipeline below title
  pipe_win = create_newwin(9,COLS,3,0,7);
  pipe_width = COLS/(NUM_STAGES + 1);
  ...
  mvwprintw(pipe_win,1,3*pipe_width-2,"IS");
  mvwprintw(pipe_win,1,4*pipe_width-2,"EX");
  mvwprintw(pipe_win,1,5*pipe_width-2,"IC");
  mvwprintw(pipe_win,1,6*pipe_width-2,"IR");
  wattroff(pipe_win,A_UNDERLINE);
  wrefresh(pipe_win);

  //instantiate window to visualize IF stage (including IF/ID)
  if_win = create_newwin((num_if_regs+2),30,12,0,5);
  mvwprintw(if_win,0,10,"IF STAGE");
  wrefresh(if_win);

  //instantiate window to visualize IF/ID signals
  if_id_win = create_newwin((num_if_id_regs+2),30,12,30,5);
  mvwprintw(if_id_win,0,12,"IF/ID");
  wrefresh(if_id_win);

  //instantiate a window to visualize ID stage
  id_win = create_newwin((num_id_regs+2),30,12,60,5);
  mvwprintw(id_win,0,10,"ID STAGE");
  wrefresh(id_win);

  //instantiate a window to visualize ID/EX signals
  is_ex_win = create_newwin((num_id_ex_regs+2),30,12,90,5);
  mvwprintw(is_ex_win,0,12,"IS/EX");
  wrefresh(is_ex_win);

  //instantiate a window to visualize EX stage
  ex_win = create_newwin((num_ex_regs+2),30,12,120,5);
  mvwprintw(ex_win,0,10,"EX STAGE");
  wrefresh(ex_win);

  //instantiate a window to visualize EX/IC
  ex_ic_win = create_newwin((num_ex_mem_regs+2),30,12,150,5);
  mvwprintw(ex_ic_win,0,12,"EX/IC");
  wrefresh(ex_ic_win);

  //instantiate a window to visualize CDB
  CDB_win = create_newwin((num_mem_wb_regs+2),30,12,180,5);
  mvwprintw(CDB_win,0,12,"IC STAGE");
  wrefresh(CDB_win);


  //instantiate a window to visualize WB stage
  wb_win = create_newwin((num_wb_regs+2),30,12,210,5);
  mvwprintw(wb_win,0,10,"IR STAGE");
  wrefresh(wb_win);

  //instantiate a window to visualize MEM stage
  mem_win = create_newwin((num_mem_regs+2),30,LINES-7-(num_mem_regs+2),0,5);
  mvwprintw(mem_win,0,10,"MEM");
  wrefresh(mem_win);

  // instantiate a window to visualize ROB
  rob_win = create_newwin((num_rob_regs+2),30,LINES-7-(num_rob_regs+2),60,5);
  mvwprintw(rob_win,0,10,"ROB");
  wrefresh(rob_win);

  // instantiate a window to visualize RS
  rs_win = create_newwin((num_rs_regs+2),30,LINES-7-(num_rs_regs+2),30,5);
...
void parsedata(int history_num_in){
  ...
  int tmp_val=0;
  char tmp_buf[32];
  int pipe_width = COLS/(NUM_STAGES + 1);
  ...
  for(j=0; j < 3; j++){
    for(i=0; i < NUM_STAGES; i++) {
      strncpy(tmp_buf,inst_contents[history_num_in]+ i*9 + (9*NUM_STAGES)*j,8);
      tmp_buf[9] = '\0';
      ...
      mvwprintw(pipe_win,2 + j,pipe_width*(i+1)-2-5,"          ");
      if (strncmp(tmp_buf,"xxxxxxxx",8) == 0)
        mvwaddnstr(pipe_win,2 + j,pipe_width*(i+1)-2-4,tmp_buf,8);
      else
        mvwaddstr(pipe_win,2 + j,pipe_width*(i+1)-2-(strlen(opcode)/2),opcode);
      if (tmp==0 || tmp==((int)'x'-(int)'0'))
        mvwprintw(pipe_win,5 + j,pipe_width*(i+1)-2,"I");
      else
        mvwprintw(pipe_win,5 + j,pipe_width*(i+1)-2,"V");
  ...
  // Handle updating the Map Table window
  int mt_num_size = MT_SIZE_IN_HEX;
  for (i=0; i < NUM_ARF; i++) {
    if (strncmp(mt_contents[history_num_in]+i*mt_num_size,
                mt_contents[old_history_num_in]+i*mt_num_size,mt_num_size))
      wattron(mt_win, A_REVERSE);
    else
      wattroff(mt_win, A_REVERSE);
    mvwaddnstr(mt_win,i+1,10,mt_contents[history_num_in]+i*mt_num_size,mt_num_size);
  }
  wrefresh(mt_win);
  ...
  for(i=0;i<num_id_ex_regs;i++){
    ...
    mvwaddstr(is_ex_win,i+1,strlen(id_ex_reg_names[i])+3,id_ex_contents[history_num_in][i]);
  }
  wrefresh(is_ex_win);
  ...
  for(i=0;i<num_ex_mem_regs;i++){
    if (strcmp(ex_mem_contents[history_num_in][i],
      ...
      wattron(ex_ic_win, A_REVERSE);
    else
      wattroff(ex_ic_win, A_REVERSE);
    mvwaddstr(ex_ic_win,i+1,strlen(ex_mem_reg_names[i])+3,ex_mem_contents[history_num_in][i]);
  }
  wrefresh(ex_ic_win);
  ...
  for(i=0;i<num_mem_wb_regs;i++){
    ...
    mvwaddstr(CDB_win,i+1,strlen(mem_wb_reg_names[i])+3,mem_wb_contents[history_num_in][i]);
  }
  wrefresh(CDB_win);
...
int processinput(){
  ...
  if(strncmp(readbuffer,"t",1) == 0){
    ...
    // We are getting Map Table registers
    strcpy(mt_contents[history_num], readbuffer+1);
    ...
    if (!setup_registers) {
      ...
      mvwaddstr(is_ex_win,id_ex_reg_num+1,1,id_ex_reg_names[id_ex_reg_num]);
      waddstr(is_ex_win, ": ");
      wrefresh(is_ex_win);
    } else {
      sscanf(readbuffer,"%*c%s %d:%s",name_buf,&tmp_len,val_buf);
    ...
    if (!setup_registers) {
      ...
      mvwaddstr(ex_ic_win,ex_mem_reg_num+1,1,ex_mem_reg_names[ex_mem_reg_num]);
      waddstr(ex_ic_win, ": ");
      wrefresh(ex_ic_win);
    } else {
      sscanf(readbuffer,"%*c%s %d:%s",name_buf,&tmp_len,val_buf);
    ...
    if (!setup_registers) {
      ...
      mvwaddstr(CDB_win,mem_wb_reg_num+1,1,mem_wb_reg_names[mem_wb_reg_num]);
      waddstr(CDB_win, ": ");
      wrefresh(CDB_win);
    } else {
      sscanf(readbuffer,"%*c%s %d:%s",name_buf,&tmp_len,val_buf);
...
extern "C" void initcurses(int if_regs, int if_id_regs, int id_regs, int id_ex_regs, int ex_regs,
                int ex_mem_regs, int mem_regs, int mem_wb_regs, int wb_regs, int rob_regs, int rs_regs,
                int misc_regs){
  int nbytes;
  int ready_val;
  ...
  if(childpid == 0){
    ...
    for(;i<NUM_HISTORY;i++){
      ...
      inst_contents[i]    = (char*) malloc(NUM_STAGES*10*3);
      arf_contents[i]     = (char*) malloc(NUM_ARF*20);
      mt_contents[i]      = (char*) malloc(NUM_ARF*20);
